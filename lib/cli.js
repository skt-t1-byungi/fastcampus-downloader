...
;(async () => {
    ...
    const pTasks = courseIds.map(async courseId => {
        ...
        const filePath = fileName => {
            ...
            return isOverwrite ? Promise.resolve(_path) : unusedFilename(_path)
        }
            ...
            .map(async (link, lectureNo) => {
                ...
                await Promise.all([
                    ...lecture.files.map(async (file, fileNo) => {
                        const dest = await filePath(filePrefix(lectureNo, fileNo) + file.name)
                    ...
                    ...lecture.mediaIds.map(async (mediaId, mediaNo) => {
                        ...
                        if (!info) return
                        const dest = await filePath(filePrefix(lectureNo, lecture.files.length + mediaNo) + info.name)
