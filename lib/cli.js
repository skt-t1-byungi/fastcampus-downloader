...
const { Client, getMediaInfo, downloadFile, downloadMedia } = require('.')
...
;(async () => {
    if (!cli.flags.all && cli.input.length === 0) panic('Required a course Id or url.')
    ...
    const pTasks = courseIds.map(async courseId => {
        ...
        const saveDir = path.join(cwd, rootDir, filenamify(course.title))
        const filePath = fileName => {
            const _path = path.join(saveDir, fileName)
            return isOverwrite ? _path : unusedFilename(_path)
        }
            ...
            .map(async (link, lectureNo) => {
                ...
                await Promise.all([
                    ...lecture.files.map(async (file, fileNo) => {
                        const dest = filePath(filePrefix(lectureNo, fileNo) + file.name)
                        await limit(() => downloadFile(file.url, dest)).catch(noop)
                        spin.text = spinText(cnt.file++)
                    }),
                    ...lecture.mediaIds.map(async (mediaId, mediaNo) => {
                        const info = await limit(() => getMediaInfo(mediaId)).catch(noop)
                        if (!info) return
                        const dest = filePath(filePrefix(lectureNo, lecture.files.length + mediaNo) + info.name)
                        await limit(() => downloadMedia(info.video, dest)).catch(noop)
    ...
    await Promise.all(pTasks).catch(err => {
        if (cli.flags.debug) throw err
        panic('Occurred unknown error.')
    })
    spin.succeed()
