...
;(async () => {
    ...
    /** @type {Client} */
    const client = await withSpinOrExit(Client.attemptLogin({ email, password }), 'Attempting login.', 'Login succeed!')

    const courseIds = cli.flags.all
        ? await withSpinOrExit(client.getCourseIds(), 'Scanning courses...', 'Scan completed.')
        : cli.input
            ...
            .map(async (link, lectureNo) => {
                ...
                await Promise.all([
                    ...
                    ...lecture.mediaIds.map(async (mediaId, mediaNo) => {
                        ...
                        await limit(() => downloadMedia(info.video.url, dest)).catch(noop)
...
function withSpinOrExit (promise, pending, succeed) {
...
function filePrefix (...numbers) {
    return numbers.map(num => String(num).padStart(3, 0) + '_').join('')
}
